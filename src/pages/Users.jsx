...
const Users = () => {
    ...
    const createUser = async () => {
        await addDoc(usersCollectionRef, { DisplayName: newDisplayName, Email: newEmail, 
            Interests: newInterests, InterestLink: newInterestLink, Age: Number(newAge) });
      };
    ...
    return (
        <div>
            ...
            <input placeholder="Display Name e.g. Donald45" onChange={(event) => {setDisplayName(event.target.value);}} /> <br></br>
            <input placeholder="Email" onChange={(event) => {setInterests(event.target.value);}} /> <br></br>
            <input placeholder="Interests" onChange={(event) => {setInterests(event.target.value);}} /> <br></br>
            <input placeholder="Interest Link" onChange={(event) => {setInterestLink(event.target.value);}} /> <br></br>
            ...
            {users.map((user) => { 
                ...
                <button onClick={() => { updateUser(user.id, user.DisplayName, user.Interests, user.InterestLink) }}>Edit</button>
